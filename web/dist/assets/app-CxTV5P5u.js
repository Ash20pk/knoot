...
\r=`.split(``);(()=>{let e=Array(128);for(let t=0;t<e.length;t+=1)e[t]=-1;for(let t=0;t<v.length;t+=1)e[v[t].charCodeAt(0)]=-2;for(let t=0;t<_.length;t+=1)e[_[t].charCodeAt(0)]=t;return e})();var Se=()=>typeof window<`u`&&typeof document<`u`,y={tested:!1,writable:!1};globalThis&&(()=>{if(!Se())return!1;try{if(typeof globalThis.localStorage!=`object`)return!1}catch{return!1}if(y.tested)return y.writable;let e=`lswt-${Math.random()}${Math.random()}`;try{globalThis.localStorage.setItem(e,e),globalThis.localStorage.removeItem(e),y.tested=!0,y.writable=!0}catch{y.tested=!0,y.writable=!1}return y.writable})()&&globalThis.localStorage&&globalThis.localStorage.getItem(`supabase.gotrue-js.locks.debug`);function Ce(){if(typeof globalThis!=`object`)try{Object.defineProperty(Object.prototype,"__magic__",{get:function(){return this},configurable:!0}),__magic__.globalThis=__magic__,delete Object.prototype.__magic__}catch{typeof self<`u`&&(self.globalThis=self)}}new class{createNewAbortSignal(){if(this.controller){let e=Error(`Cancelling existing WebAuthn API call for new one`);e.name=`AbortError`,this.controller.abort(e)}let e=new AbortController;return this.controller=e,e.signal}cancelCeremony(){if(this.controller){let e=Error(`Manually cancelling existing WebAuthn API call`);e.name=`AbortError`,this.controller.abort(e),this.controller=void 0}}},Ce();var we=`2.114.0`,b=``,x;if(typeof Deno<`u`)b=`deno`,x=Deno.version?.deno;else if(typeof document<`u`)b=`web`;else if(typeof navigator<`u`&&navigator.product===`ReactNative`)b=`react-native`;else{var S;b=`node`;let e=globalThis.process;x=e==null||(S=e.version)==null?void 0:S.replace(/^v/,``)}var C=[`runtime=${b}`];x&&C.push(`runtime-version=${x}`),`${we}${C.join(`; `)}`;function Te(){if(typeof window<`u`||globalThis.Deno!==void 0)return!1;let e=globalThis.process;if(!e)return!1;let t=e.version;if(t==null)return!1;let n=t.match(/^v(\d+)\./);return n?parseInt(n[1],10)<=20:!1}Te()&&console.warn(`⚠️  Node.js 20 and below are deprecated and will no longer be supported in future versions of @supabase/supabase-js. Please upgrade to Node.js 22 or later. For more information, visit: https://github.com/orgs/supabase/discussions/45715`);var w=null;function T(){throw Error(`Sign-in is not configured on this deployment.`)}async function Ee(e){let{data:t,error:n}=await T().rpc(`create_team`,{team_name:e});if(n)throw Error(n.message);return t}async function De(e,t=`member`){let{data:n,error:r}=await T().rpc(`invite_member`,{invite_email:e,invite_role:t});if(r)throw Error(r.message);return n}async function E(){let{data:e,error:t}=await T().from(`invites`).select(`id, email, role, created_at, expires_at`).is(`accepted_at`,null).order(`created_at`);if(t)throw Error(t.message);return e??[]}async function Oe(e){let{error:t}=await T().rpc(`revoke_invite`,{invite_id:e});if(t)throw Error(t.message)}async function D(){throw Error(`Sign-in is not configured on this deployment.`)}async function O(e,t={}){let n={Authorization:`Bearer ${await D()}`};t.body&&(n[`Content-Type`]=`application/json`);let r=await fetch(e,{...t,headers:{...n,...t.headers}}),i=await r.json().catch(()=>({}));if(!r.ok)throw Error(i.error||`${r.status} ${r.statusText}`);return i}async function ke(){let e=await D();return new WebSocket(`${n}?token=${encodeURIComponent(e)}`)}var Ae=class{onEvent;onState;onConn;claims=[];sessions=new Map;events=[];ws=null;gen=0;repo=null;stopped=!1;constructor(e,t,n){this.onEvent=e,this.onState=t,this.onConn=n}async open(e){let t=++this.gen;this.repo=e,this.close(!1),this.claims=[],this.sessions.clear(),this.events=[];try{let n=await O(`/api/events?repo=${encodeURIComponent(e)}&limit=300`);if(t!==this.gen)return;this.events=n}catch{}this.onState();let n;try{n=await ke()}catch{this.onConn(!1);return}if(t!==this.gen){n.close();return}this.ws=n,n.onopen=()=>{if(t!==this.gen){n.close();return}this.onConn(!0),n.send(JSON.stringify({type:`hello`,repo:e,daemon:`console`}))},n.onmessage=e=>{if(t!==this.gen)return;let n;try{n=JSON.parse(e.data)}catch{return}n.type===`welcome`?(this.claims=n.claims??[],this.sessions=new Map((n.sessions??[]).map(e=>[e.session,e])),this.onState()):n.type===`event`&&n.event&&(this.apply(n.event),this.events.push(n.event),this.events.length>500&&this.events.shift(),this.onEvent(n.event),this.onState())},n.onerror=()=>{t===this.gen&&this.onConn(!1)},n.onclose=()=>{t!==this.gen||this.stopped||(this.onConn(!1),setTimeout(()=>{t===this.gen&&this.repo&&!this.stopped&&this.open(this.repo)},3e3))}}close(e=!0){if(e&&(this.stopped=!0,this.gen++),this.ws){try{this.ws.close()}catch{}this.ws=null}}apply(e){switch(e.type){case`claim_acquired`:this.claims=this.claims.filter(t=>t.path!==e.path),this.claims.push({session:e.session,user:e.user,path:e.path,intent:e.intent,lease_until:e.lease_until});break;case`claim_released`:case`path_freed`:this.claims=this.claims.filter(t=>t.path!==e.path);break;case`session_started`:this.sessions.set(e.session,{session:e.session,user:e.user,branch:e.branch,intent:``});break;case`intent_declared`:{let t=this.sessions.get(e.session);t&&(t.intent=e.text??``);break}case`session_ended`:this.sessions.delete(e.session),this.claims=this.claims.filter(t=>t.session!==e.session)}}},je={claim_acquired:`held`,claim_denied:`blocked`,claim_released:`plain`,path_freed:`wire`,message:`wire`,ungated_write:`warn`,cross_branch_overlap:`warn`,create_collision:`blocked`,duplicate_intent:`blocked`,stale_read:`warn`,path_removed:`warn`};function Me(e){switch(e.type){case`claim_denied`:return`${e.path}, held by ${e.holder_user??`unknown`}`;case`ungated_write`:return`${e.path}, over ${e.holder_user??`a peer`}’s claim`;case`intent_declared`:return`“${e.text??``}”`;case`message`:return`to ${e.to??`all`}: ${e.text??``}`;case`session_started`:return`joined on ${e.branch??`unknown branch`}`;case`session_ended`:return`left`;case`stale_read`:return`${e.path}, read before ${e.peer_user??`a peer`} changed it`;case`create_collision`:return`${e.path}, already created by ${e.peer_user??`a peer`}`;case`duplicate_intent`:return`same task as ${e.peer_user??`a peer`}: “${e.peer_text??``}”`;case`path_removed`:return`${e.path} ${e.moved?`moved away`:`deleted`}`;default:return e.path??``}}var k=e=>{if(!e)return`never`;let t=Date.now()-e;return t<6e4?`just now`:t<36e5?`${Math.floor(t/6e4)} min ago`:t<864e5?`${Math.floor(t/36e5)} h ago`:`${Math.floor(t/864e5)} d ago`},A=(e,t=document)=>t.querySelector(e),Ne=A(`#auth`),j=A(`#shell`),Pe=A(`#booting`),M=A(`#view`),N=null,P=`member`,F=null,I=null,L=null,R=`knoot.pendingTeam`,Fe=e=>{try{e&&localStorage.setItem(R,e)}catch{}},Ie=()=>{try{let e=localStorage.getItem(R);return localStorage.removeItem(R),e}catch{return null}},z=location.hash===`#signup`?`signup`:`signin`;function B(){let e=z===`signup`;A(`#auth-title`).textContent=e?`Create your account`:`Sign in`,A(`#auth-sub`).textContent=e?`A team, an agent token, and a live log of every session. No card needed.`:`Manage your team, agent tokens and live sessions.`,A(`#auth-go`).textContent=e?`Create account`:`Sign in`,A(`#auth-switch`).textContent=e?`I already have an account`:`Create an account`,A(`#team-field`).hidden=!e,A(`#auth-team`).required=e,A(`#auth-password`).autocomplete=e?`new-password`:`current-password`}function V(e,t=``){let n=A(`#auth-err`),r=A(`#auth-ok`);n.hidden=e!==`err`,r.hidden=e!==`ok`,e===`err`&&(n.textContent=t),e===`ok`&&(r.textContent=t)}function H(){Pe.hidden=!0,j.hidden=!0,Ne.hidden=!1,B(),V(`err`,`Sign-in is not configured on this deployment. Set VITE_SUPABASE_URL and VITE_SUPABASE_PUBLISHABLE_KEY at build time, or run your own relay and use an agent token.`),A(`#auth-go`).disabled=!0}A(`#auth-switch`).addEventListener(`click`,()=>{z=z===`signup`?`signin`:`signup`,location.hash=z===`signup`?`#signup`:``,V(`clear`),B()}),A(`#auth-reset`).addEventListener(`click`,async()=>{let e=A(`#auth-email`).value.trim();if(!e){V(`err`,`Enter your email address first, then choose Forgot password.`);return}try{let{error:t}=await w.auth.resetPasswordForEmail(e,{redirectTo:`${location.origin}/app/`});if(t)throw Error(t.message);V(`ok`,`Check ${e} for a link to set a new password.`)}catch(e){V(`err`,e.message)}}),A(`#auth-form`).addEventListener(`submit`,async e=>{e.preventDefault();let t=A(`#auth-go`),n=A(`#auth-email`).value.trim(),r=A(`#auth-password`).value,i=A(`#auth-team`).value.trim();V(`clear`),t.disabled=!0,t.textContent=z===`signup`?`Creating account`:`Signing in`;try{let e=w;if(z===`signup`){let{data:t,error:a}=await e.auth.signUp({email:n,password:r});if(a)throw Error(a.message);if(Fe(i),!t.session){V(`ok`,`Check ${n} to confirm your address, then sign in. Your team is created when you first sign in.`),z=`signin`,B();return}await Ee(i||`${n.split(`@`)[0]}'s team`),Ie()}else{let{error:t}=await e.auth.signInWithPassword({email:n,password:r});if(t)throw Error(t.message)}await $()}catch(e){V(`err`,e.message)}finally{t.disabled=!1,B()}}),A(`#signout`).addEventListener(`click`,async()=>{I?.close(),await w?.auth.signOut(),N=null,location.hash=``,H()});var Le=[`sessions`,`repositories`,`tokens`,`rooms`,`team`,`settings`];function U(){let e=location.hash.replace(`#`,``);return Le.includes(e)?e:`sessions`}function Re(){let e=U();for(let t of document.querySelectorAll(`.tabs a`))t.classList.toggle(`on`,t.getAttribute(`href`)===`#${e}`)}function W(){switch(Re(),I?.close(),I=null,U()){case`sessions`:return ze();case`repositories`:return He();case`tokens`:return Y();case`rooms`:return X();case`team`:return Z();case`settings`:return Ue()}}function ze(){let n=F?.repos??[];if(M.innerHTML=`
    <div class="page">
      <div class="page-head">
        <div>
          <h1>Sessions</h1>
    ...
    </div>`,!n.length){A(`#log-rows`).innerHTML=G(),e(M);return}let r=A(`#repo-pick`);L&&n.some(e=>e.repo===L)&&(r.value=L),L=r.value,r.addEventListener(`change`,()=>{L=r.value,K()}),K()}function G(){return`<div class="empty">
  ...
  </div>`}function K(){let e=A(`#log-rows`),t=A(`#log`);I?.close(),I=new Ae(n=>Be(n,e,t),()=>{Ve(),e.dataset.seeded||J(e,t)},e=>{let t=A(`#conn`);t.textContent=e===!0?`live`:e===!1?`reconnecting`:`idle`,t.className=`state`+(e===!0?` live`:e===!1?` off`:``)}),I.open(L)}function q(e,n){let r=je[e.type]??`plain`,i=e.ts?new Date(e.ts).toLocaleTimeString([],{hour12:!1}).slice(0,8):``;return`<div class="row${r===`blocked`?` is-blocked`:``}${n?` enter`:``}">
    <span class="t">${t(i)}</span><span class="u">${t(e.user??``)}</span>
    <span class="k ${r}">${t(e.type)}</span><span class="d">${t(Me(e))}</span></div>`}function J(e,t){let n=I?.events??[];e.innerHTML=n.length?n.slice(-300).map(e=>q(e,!1)).join(``):`<div class="empty">Connected. Nothing has happened in this repository yet.</div>`,e.dataset.seeded=`1`,t.scrollTop=t.scrollHeight}function Be(e,t,n){let r=n.scrollTop+n.clientHeight>=n.scrollHeight-30;for(t.querySelector(`.empty`)&&(t.innerHTML=``),t.insertAdjacentHTML(`beforeend`,q(e,!0));t.children.length>300;)t.removeChild(t.firstChild);r&&(n.scrollTop=n.scrollHeight)}function Ve(){let e=A(`#presence`);if(!e||!I)return;let n=[...I.sessions.values()],r=new Set(I.events.slice(-60).filter(e=>e.type===`claim_denied`).map(e=>e.session));e.innerHTML=n.length?`<table class="rows">
        <thead><tr><th>Agent</th><th>Working on</th><th>Holds</th></tr></thead>
        <tbody>${n.map(e=>{let n=I.claims.filter(t=>t.session===e.session).map(e=>e.path),i=r.has(e.session)&&!n.length,a=n.length?`holds`:i?`holds blocked`:`holds none`,o=n.length?n.join(`  `):i?`blocked, waiting`:`nothing`;return`<tr><td class="mono">${t(e.user??e.session.slice(0,8))}</td>
            <td class="dim">${t(e.intent||`no stated intent yet`)}</td>
            <td class="${a}">${t(o)}</td></tr>`}).join(``)}</tbody></table>`:``;let i=n.length,a=I.claims.length,o=r.size,s=A(`#counts`);s&&(s.innerHTML=`<span><b>${i}</b>session${i===1?``:`s`}</span><span><b>${a}</b>claim${a===1?``:`s`}</span>`+(o?`<span class="blocked"><b>${o}</b>blocked</span>`:``))}function He(){let n=F?.repos??[];M.innerHTML=`
    <div class="page">
      <div class="page-head">
        <div>
          <h1>Repositories</h1>
      ...
      <div class="panel">
        ${n.length?`<table class="rows">
          ...
          <tbody>${n.map(e=>`<tr>
            ...
            <td class="dim">${t(k(e.last_seen_ts??null))}</td>
            <td class="right"><a class="btn quiet sm" href="#sessions" data-repo="${t(e.repo)}">Open log</a></td>
          </tr>`).join(``)}</tbody></table>`:G()}
      </div>
    </div>`;for(let e of M.querySelectorAll(`[data-repo]`))e.addEventListener(`click`,()=>{L=e.dataset.repo});e(M)}function Y(){let r=F?.tokens??[],i=r.filter(e=>!e.revoked).length,a=F?.members??[],o=a.filter(e=>e.unassigned),s=F?.me;M.innerHTML=`
    <div class="page">
      <div class="page-head">
        <div>
          <h1>Agent keys</h1>
      ...
      <div class="panel">
        ...
        ${r.length?`<table class="rows">
          ...
          <tbody>${r.map(e=>`<tr>
            ...
            <td class="dim">${t(k(e.created_ts))}</td>
            <td class="dim">${e.revoked?``:t(k(e.last_seen_ts))}</td>
      ...
      <div class="panel">
        ...
        <div class="panel-body steps">
          ...
          <div class="step"><p>Store the key on that machine and run the daemon. <code>join</code> asks the relay who the key is for and prints the rooms and areas it opens, so a wrong key fails here rather than quietly an hour later.</p>
            <div class="cmd-row"><code>knoot join &lt;key&gt; --relay ${t(n)}</code><button class="copy" type="button">Copy</button></div>
            <div class="cmd-row"><code>knoot daemon</code><button class="copy" type="button">Copy</button></div></div>
        </div>
      </div>
    </div>`,e(M),A(`#mint-go`).addEventListener(`click`,async()=>{let r=A(`#mint-go`),i=A(`#tok-err`);i.hidden=!0,r.disabled=!0;try{let r=A(`#mint-label`).value.trim(),i=A(`#mint-member`)?.value,a=await O(`/api/tokens`,{method:`POST`,body:JSON.stringify(i?{label:r,member:i}:{label:r})});A(`#mint-out`).innerHTML=`<div class="reveal">
        <div class="lbl">New key. This is the only time it is readable.</div>
        <div class="val">${t(a.token)}</div></div>
        <div class="cmd-row"><code>knoot join ${t(a.token)} --relay ${t(n)}</code><button class="copy" type="button">Copy</button></div>`,e(A(`#mint-out`)),await Q()}catch(e){i.textContent=e.message,i.hidden=!1}finally{r.disabled=!1}});for(let e of M.querySelectorAll(`[data-attach]`))e.addEventListener(`click`,async()=>{e.disabled=!0;try{await O(`/api/members/attach`,{method:`POST`,body:JSON.stringify({from:e.dataset.attach})}),await Q(),Y()}catch(t){let n=A(`#tok-err`);n.textContent=t.message,n.hidden=!1,e.disabled=!1}});for(let e of M.querySelectorAll(`[data-revoke]`))e.addEventListener(`click`,async()=>{if(confirm(`Revoke this key? The machine using it stops coordinating. It fails open, so its agents keep working alone.`)){e.disabled=!0;try{await O(`/api/tokens/${encodeURIComponent(e.dataset.revoke)}/revoke`,{method:`POST`}),await Q(),Y()}catch(t){let n=A(`#tok-err`);n.textContent=t.message,n.hidden=!1,e.disabled=!1}}})}function X(){let e=F?.rooms??[],n=F?.members??[],r=F?.repos??[],i=F?.me,a=i?.role===`owner`||i?.role===`admin`,o=e=>e.repo===`*`&&e.area===`/`?`every repository`:`${e.repo}:${e.area}`;M.innerHTML=`
    <div class="page">
      <div class="page-head">
        <div>
          <h1>Rooms</h1>
        ...
        <div class="panel-body">
          ...
          ${a?`<div class="inline-form" style="margin-top:12px">
            <select data-member-pick="${t(e.id)}">
              ${n.filter(t=>!e.members.some(e=>e.id===t.id)).map(e=>`<option value="${t(e.id)}">${t(e.email)}</option>`).join(``)}
            </select>
    ...
    </div>`;let s=(e,t)=>{let n=M.querySelector(`[data-room-err="${e}"]`);n&&(n.textContent=t.message,n.hidden=!1)},c=async()=>{await Q(),X()};A(`#room-go`)?.addEventListener(`click`,async()=>{let e=A(`#room-err`);e.hidden=!0;let t=A(`#room-name`).value.trim();if(!t){e.textContent=`A room needs a name.`,e.hidden=!1;return}try{await O(`/api/rooms`,{method:`POST`,body:JSON.stringify({name:t})}),await c()}catch(t){e.textContent=t.message,e.hidden=!1}});for(let e of M.querySelectorAll(`[data-add-area]`))e.addEventListener(`click`,async()=>{let t=e.dataset.addArea,n=M.querySelector(`[data-area-repo="${t}"]`).value,r=M.querySelector(`[data-area-path="${t}"]`).value.trim()||`/`;try{await O(`/api/rooms/${encodeURIComponent(t)}/areas`,{method:`POST`,body:JSON.stringify({repo:n,area:r})}),await c()}catch(e){s(t,e)}});for(let e of M.querySelectorAll(`[data-rm-area]`))e.addEventListener(`click`,async()=>{let t=e.dataset.rmArea;try{await O(`/api/rooms/${encodeURIComponent(t)}/areas`,{method:`POST`,body:JSON.stringify({repo:e.dataset.repo,area:e.dataset.area,remove:!0})}),await c()}catch(e){s(t,e)}});for(let e of M.querySelectorAll(`[data-add-member]`))e.addEventListener(`click`,async()=>{let t=e.dataset.addMember,n=M.querySelector(`[data-member-pick="${t}"]`)?.value;if(!n){s(t,Error(`Everyone in the team is already in this room.`));return}try{await O(`/api/rooms/${encodeURIComponent(t)}/members`,{method:`POST`,body:JSON.stringify({member:n})}),await c()}catch(e){s(t,e)}});for(let e of M.querySelectorAll(`[data-rm-member]`))e.addEventListener(`click`,async()=>{let t=e.dataset.rmMember;try{await O(`/api/rooms/${encodeURIComponent(t)}/members`,{method:`POST`,body:JSON.stringify({member:e.dataset.member,remove:!0})}),await c()}catch(e){s(t,e)}});for(let e of M.querySelectorAll(`[data-del-room]`))e.addEventListener(`click`,async()=>{let t=e.dataset.delRoom;if(confirm(`Delete this room? The people in it keep their keys; they stop sharing the areas this room held.`))try{await O(`/api/rooms/${encodeURIComponent(t)}/delete`,{method:`POST`}),await c()}catch(e){s(t,e)}})}async function Z(){let n=P===`owner`||P===`admin`,r=F?.members??[];M.innerHTML=`
    <div class="page">
      <div class="page-head">
        <div>
          <h1>Team</h1>
          <p>Everyone here can see the log and hold keys of their own. You are signed in as ${t(P)}.</p>
        </div>
      </div>
      <div class="panel">
    ...
    </div>`,e(M);let i=e=>r.find(t=>t.email.toLowerCase()===e.toLowerCase()),a=async()=>{try{let e=await E();A(`#invites`).innerHTML=e.length?`<table class="rows"><thead><tr><th>Email</th><th>Role</th><th>Expires</th><th></th></tr></thead>
           <tbody>${e.map(e=>`<tr>
             <td>${t(e.email)}</td><td class="dim">${t(e.role)}</td>
             <td class="dim">${t(k(Date.parse(e.expires_at)))}</td>
             <td class="right">${n?`<button class="btn quiet sm" data-inv-revoke="${t(e.id)}">Withdraw</button>`:``}</td>
           </tr>`).join(``)}</tbody></table>`:`<div class="empty">None outstanding.</div>`;for(let e of M.querySelectorAll(`[data-inv-revoke]`))e.addEventListener(`click`,async()=>{e.disabled=!0;try{await Oe(e.dataset.invRevoke),await a()}catch(t){alert(t.message),e.disabled=!1}})}catch(e){A(`#invites`).innerHTML=`<div class="empty">Could not load invitations: ${t(e.message)}</div>`}},o=async()=>{try{let e=r.filter(e=>!e.unassigned).map(e=>({user_id:``,email:e.email,role:e.role,created_at:``}));A(`#members`).innerHTML=e.length?`<table class="rows"><thead><tr><th>Email</th><th>Role</th><th>Joined</th><th>Keys</th><th></th></tr></thead>
           <tbody>${e.map(e=>{let r=i(e.email),a=(F?.tokens??[]).filter(e=>r&&e.member_id===r.id&&!e.revoked).length;return`<tr><td>${t(e.email)}</td><td class="dim">${t(e.role)}</td>
               <td class="dim">${e.created_at?t(k(Date.parse(e.created_at))):`&mdash;`}</td>
               <td class="dim">${a}</td>
               <td class="right">${n&&e.role!==`owner`?`<button class="btn danger sm" data-remove="${t(e.user_id)}" data-email="${t(e.email)}" data-member="${t(r?.id??``)}">Remove</button>`:``}</td></tr>`}).join(``)}</tbody></table>`:`<div class="empty">Just you so far.</div>`;for(let e of M.querySelectorAll(`[data-remove]`))e.addEventListener(`click`,async()=>{if(confirm(`Remove ${e.dataset.email}? Their keys stop working at once. Nobody else's key changes.`)){e.disabled=!0;try{e.dataset.member&&await O(`/api/members/${encodeURIComponent(e.dataset.member)}/remove`,{method:`POST`}),await Q(),await Z()}catch(t){alert(t.message),e.disabled=!1}}})}catch(e){A(`#members`).innerHTML=`<div class="empty">Could not load members: ${t(e.message)}</div>`}};A(`#add-go`)?.addEventListener(`click`,async()=>{let n=A(`#add-err`);n.hidden=!0;let r=A(`#add-email`).value.trim(),i=A(`#add-label`).value.trim()||`first machine`,a=A(`#add-role`).value;if(!r.includes(`@`)){n.textContent=`That does not look like an email address.`,n.hidden=!1;return}try{let s=await O(`/api/members`,{method:`POST`,body:JSON.stringify({email:r,role:a,label:i})});if(s.existing){n.textContent=`${s.email} is already on this team as ${s.role}. Mint a key for them under Agent keys.`,n.hidden=!1;return}A(`#add-out`).innerHTML=`<div class="reveal">
          <div class="lbl">${t(s.email)}&rsquo;s key. Readable only now.</div>
          <div class="val">${t(s.token??``)}</div>
        </div>
        <div class="cmd-row"><code>knoot join ${t(s.token??``)}</code><button class="copy" type="button">Copy</button></div>`,e(A(`#add-out`)),A(`#add-email`).value=``,await Q(),await o()}catch(e){n.textContent=e.message,n.hidden=!1}}),A(`#inv-go`)?.addEventListener(`click`,async()=>{let n=A(`#inv-err`);n.hidden=!0;let r=A(`#inv-email`).value.trim(),i=A(`#inv-role`).value;if(!r.includes(`@`)){n.textContent=`That does not look like an email address.`,n.hidden=!1;return}try{let n=await De(r,i),o=`${location.origin}/app/#join=${n}`;A(`#inv-out`).innerHTML=`<div class="reveal">
          <div class="lbl">Send ${t(r)} this link. It is readable only now.</div>
          <div class="val">${t(o)}</div>
        </div>
        <div class="cmd-row"><code>${t(o)}</code><button class="copy" type="button">Copy</button></div>`,e(A(`#inv-out`)),A(`#inv-email`).value=``,await a()}catch(e){n.textContent=e.message,n.hidden=!1}}),await o()}function Ue(){M.innerHTML=`
    <div class="page">
      ...
      <div class="panel">
        ...
        <div class="panel-body">
          ...
          <p style="margin-top:16px">Team id <code>${t(F?.team_id??``)}</code></p>
        </div>
      </div>
      <div class="panel">
    ...
    </div>`,e(M),A(`#pw-go`).addEventListener(`click`,async()=>{let e=A(`#pw-err`),t=A(`#pw-ok`);e.hidden=!0,t.hidden=!0;let n=A(`#new-password`).value;if(n.length<8){e.textContent=`Use at least 8 characters.`,e.hidden=!1;return}let{error:r}=await w.auth.updateUser({password:n});if(r){e.textContent=r.message,e.hidden=!1;return}t.textContent=`Password changed.`,t.hidden=!1,A(`#new-password`).value=``})}async function Q(){F=await O(`/api/team`)}async function $(){H()}addEventListener(`hashchange`,()=>{if(j.hidden){z=location.hash===`#signup`?`signup`:`signin`,B();return}W()}),setInterval(async()=>{if(!j.hidden&&N)try{let e=(F?.repos??[]).map(e=>e.repo).join();await Q();let t=(F?.repos??[]).map(e=>e.repo).join();(e!==t&&U()!==`sessions`||e!==t&&!L)&&W()}catch{}},2e4),$();