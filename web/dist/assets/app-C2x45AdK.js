...
\r=`.split(``);(()=>{let e=Array(128);for(let t=0;t<e.length;t+=1)e[t]=-1;for(let t=0;t<_.length;t+=1)e[_[t].charCodeAt(0)]=-2;for(let t=0;t<g.length;t+=1)e[g[t].charCodeAt(0)]=t;return e})();var Ce=()=>typeof window<`u`&&typeof document<`u`,v={tested:!1,writable:!1};globalThis&&(()=>{if(!Ce())return!1;try{if(typeof globalThis.localStorage!=`object`)return!1}catch{return!1}if(v.tested)return v.writable;let e=`lswt-${Math.random()}${Math.random()}`;try{globalThis.localStorage.setItem(e,e),globalThis.localStorage.removeItem(e),v.tested=!0,v.writable=!0}catch{v.tested=!0,v.writable=!1}return v.writable})()&&globalThis.localStorage&&globalThis.localStorage.getItem(`supabase.gotrue-js.locks.debug`);function we(){if(typeof globalThis!=`object`)try{Object.defineProperty(Object.prototype,"__magic__",{get:function(){return this},configurable:!0}),__magic__.globalThis=__magic__,delete Object.prototype.__magic__}catch{typeof self<`u`&&(self.globalThis=self)}}new class{createNewAbortSignal(){if(this.controller){let e=Error(`Cancelling existing WebAuthn API call for new one`);e.name=`AbortError`,this.controller.abort(e)}let e=new AbortController;return this.controller=e,e.signal}cancelCeremony(){if(this.controller){let e=Error(`Manually cancelling existing WebAuthn API call`);e.name=`AbortError`,this.controller.abort(e),this.controller=void 0}}},we();var Te=`2.114.0`,y=``,b;if(typeof Deno<`u`)y=`deno`,b=Deno.version?.deno;else if(typeof document<`u`)y=`web`;else if(typeof navigator<`u`&&navigator.product===`ReactNative`)y=`react-native`;else{var x;y=`node`;let e=globalThis.process;b=e==null||(x=e.version)==null?void 0:x.replace(/^v/,``)}var S=[`runtime=${y}`];b&&S.push(`runtime-version=${b}`),`${Te}${S.join(`; `)}`;function Ee(){if(typeof window<`u`||globalThis.Deno!==void 0)return!1;let e=globalThis.process;if(!e)return!1;let t=e.version;if(t==null)return!1;let n=t.match(/^v(\d+)\./);return n?parseInt(n[1],10)<=20:!1}Ee()&&console.warn(`⚠️  Node.js 20 and below are deprecated and will no longer be supported in future versions of @supabase/supabase-js. Please upgrade to Node.js 22 or later. For more information, visit: https://github.com/orgs/supabase/discussions/45715`);var C=null;function w(){throw Error(`Sign-in is not configured on this deployment.`)}async function De(e){let{data:t,error:n}=await w().rpc(`create_team`,{team_name:e});if(n)throw Error(n.message);return t}async function Oe(e,t=`member`){let{data:n,error:r}=await w().rpc(`invite_member`,{invite_email:e,invite_role:t});if(r)throw Error(r.message);return n}async function ke(){let{data:e,error:t}=await w().from(`invites`).select(`id, email, role, created_at, expires_at`).is(`accepted_at`,null).order(`created_at`);if(t)throw Error(t.message);return e??[]}async function Ae(e){let{error:t}=await w().rpc(`revoke_invite`,{invite_id:e});if(t)throw Error(t.message)}async function T(){throw Error(`Sign-in is not configured on this deployment.`)}async function E(e,t={}){let n={Authorization:`Bearer ${await T()}`};t.body&&(n[`Content-Type`]=`application/json`);let r=await fetch(e,{...t,headers:{...n,...t.headers}}),i=await r.json().catch(()=>({}));if(!r.ok)throw Error(i.error||`${r.status} ${r.statusText}`);return i}async function je(){let e=await T();return new WebSocket(`${n}?token=${encodeURIComponent(e)}`)}var Me=class{onEvent;onState;onConn;claims=[];sessions=new Map;events=[];ws=null;gen=0;repo=null;stopped=!1;constructor(e,t,n){this.onEvent=e,this.onState=t,this.onConn=n}async open(e){let t=++this.gen;this.repo=e,this.close(!1),this.claims=[],this.sessions.clear(),this.events=[];try{let n=await E(`/api/events?repo=${encodeURIComponent(e)}&limit=300`);if(t!==this.gen)return;this.events=n}catch{}this.onState();let n;try{n=await je()}catch{this.onConn(!1);return}if(t!==this.gen){n.close();return}this.ws=n,n.onopen=()=>{if(t!==this.gen){n.close();return}this.onConn(!0),n.send(JSON.stringify({type:`hello`,repo:e,daemon:`console`}))},n.onmessage=e=>{if(t!==this.gen)return;let n;try{n=JSON.parse(e.data)}catch{return}n.type===`welcome`?(this.claims=n.claims??[],this.sessions=new Map((n.sessions??[]).map(e=>[e.session,e])),this.onState()):n.type===`event`&&n.event&&(this.apply(n.event),this.events.push(n.event),this.events.length>500&&this.events.shift(),this.onEvent(n.event),this.onState())},n.onerror=()=>{t===this.gen&&this.onConn(!1)},n.onclose=()=>{t!==this.gen||this.stopped||(this.onConn(!1),setTimeout(()=>{t===this.gen&&this.repo&&!this.stopped&&this.open(this.repo)},3e3))}}close(e=!0){if(e&&(this.stopped=!0,this.gen++),this.ws){try{this.ws.close()}catch{}this.ws=null}}apply(e){switch(e.type){case`claim_acquired`:this.claims=this.claims.filter(t=>t.path!==e.path),this.claims.push({session:e.session,user:e.user,path:e.path,intent:e.intent,lease_until:e.lease_until});break;case`claim_released`:case`path_freed`:this.claims=this.claims.filter(t=>t.path!==e.path);break;case`session_started`:this.sessions.set(e.session,{session:e.session,user:e.user,branch:e.branch,intent:``});break;case`intent_declared`:{let t=this.sessions.get(e.session);t&&(t.intent=e.text??``);break}case`session_ended`:this.sessions.delete(e.session),this.claims=this.claims.filter(t=>t.session!==e.session)}}},Ne={claim_acquired:`held`,claim_denied:`blocked`,claim_released:`plain`,path_freed:`wire`,message:`wire`,ungated_write:`warn`,cross_branch_overlap:`warn`,create_collision:`blocked`,duplicate_intent:`blocked`,stale_read:`warn`,path_removed:`warn`};function D(e){switch(e.type){case`claim_denied`:return`${e.path}, held by ${e.holder_user??`unknown`}`;case`ungated_write`:return`${e.path}, over ${e.holder_user??`a peer`}’s claim`;case`intent_declared`:return`“${e.text??``}”`;case`message`:return`to ${e.to??`all`}: ${e.text??``}`;case`session_started`:return`joined on ${e.branch??`unknown branch`}`;case`session_ended`:return`left`;case`stale_read`:return`${e.path}, read before ${e.peer_user??`a peer`} changed it`;case`create_collision`:return`${e.path}, already created by ${e.peer_user??`a peer`}`;case`duplicate_intent`:return`same task as ${e.peer_user??`a peer`}: “${e.peer_text??``}”`;case`path_removed`:return`${e.path} ${e.moved?`moved away`:`deleted`}`;default:return e.path??``}}var O=e=>{if(!e)return`never`;let t=Date.now()-e;return t<6e4?`just now`:t<36e5?`${Math.floor(t/6e4)} min ago`:t<864e5?`${Math.floor(t/36e5)} h ago`:`${Math.floor(t/864e5)} d ago`},k=(e,t=document)=>t.querySelector(e),Pe=k(`#auth`),A=k(`#shell`),Fe=k(`#booting`),j=k(`#view`),M=null,N=`member`,P=null,F=null,I=null,L=`knoot.pendingTeam`,Ie=e=>{try{e&&localStorage.setItem(L,e)}catch{}},Le=()=>{try{let e=localStorage.getItem(L);return localStorage.removeItem(L),e}catch{return null}},R=location.hash===`#signup`?`signup`:`signin`;function z(){let e=R===`signup`;k(`#auth-title`).textContent=e?`Create your account`:`Sign in`,k(`#auth-sub`).textContent=e?`A team, an agent token, and a live log of every session. No card needed.`:`Manage your team, agent tokens and live sessions.`,k(`#auth-go`).textContent=e?`Create account`:`Sign in`,k(`#auth-switch`).textContent=e?`I already have an account`:`Create an account`,k(`#team-field`).hidden=!e,k(`#auth-team`).required=e,k(`#auth-password`).autocomplete=e?`new-password`:`current-password`}function B(e,t=``){let n=k(`#auth-err`),r=k(`#auth-ok`);n.hidden=e!==`err`,r.hidden=e!==`ok`,e===`err`&&(n.textContent=t),e===`ok`&&(r.textContent=t)}function V(){Fe.hidden=!0,A.hidden=!0,Pe.hidden=!1,z(),B(`err`,`Sign-in is not configured on this deployment. Set VITE_SUPABASE_URL and VITE_SUPABASE_PUBLISHABLE_KEY at build time, or run your own relay and use an agent token.`),k(`#auth-go`).disabled=!0}k(`#auth-switch`).addEventListener(`click`,()=>{R=R===`signup`?`signin`:`signup`,location.hash=R===`signup`?`#signup`:``,B(`clear`),z()}),k(`#auth-reset`).addEventListener(`click`,async()=>{let e=k(`#auth-email`).value.trim();if(!e){B(`err`,`Enter your email address first, then choose Forgot password.`);return}try{let{error:t}=await C.auth.resetPasswordForEmail(e,{redirectTo:`${location.origin}/app/`});if(t)throw Error(t.message);B(`ok`,`Check ${e} for a link to set a new password.`)}catch(e){B(`err`,e.message)}}),k(`#auth-form`).addEventListener(`submit`,async e=>{e.preventDefault();let t=k(`#auth-go`),n=k(`#auth-email`).value.trim(),r=k(`#auth-password`).value,i=k(`#auth-team`).value.trim();B(`clear`),t.disabled=!0,t.textContent=R===`signup`?`Creating account`:`Signing in`;try{let e=C;if(R===`signup`){let{data:t,error:a}=await e.auth.signUp({email:n,password:r});if(a){if(/already|registered|exists/i.test(a.message)){R=`signin`,location.hash=``,z(),B(`ok`,`${n} already has an account. Sign in with its password, or choose Forgot password.`),k(`#auth-password`).focus();return}throw Error(a.message)}if(Ie(i),!t.session){B(`ok`,`Check ${n} to confirm your address, then sign in. Your team is created when you first sign in.`),R=`signin`,z();return}await De(i||`${n.split(`@`)[0]}'s team`),Le()}else{let{error:t}=await e.auth.signInWithPassword({email:n,password:r});if(t)throw Error(t.message)}await it()}catch(e){B(`err`,e.message)}finally{t.disabled=!1,z()}}),k(`#signout`).addEventListener(`click`,async()=>{F?.close(),await C?.auth.signOut(),M=null,location.hash=``,V()});var Re=[`start`,`sessions`,`memory`,`history`,`repositories`,`tokens`,`rooms`,`team`,`settings`],ze=()=>!!P?.repos?.length;function H(){let e=location.hash.replace(`#`,``);return Re.includes(e)?e:ze()?`sessions`:`start`}function U(){let e=H();for(let t of document.querySelectorAll(`.tabs a`))t.classList.toggle(`on`,t.getAttribute(`href`)===`#${e}`);k(`#tab-start`)?.classList.toggle(`done`,q().current===0)}function W(){switch(U(),F?.close(),F=null,K(),H()){case`start`:return Ve();case`sessions`:return Be();case`memory`:return Je();case`history`:return Ze();case`repositories`:return et();case`tokens`:return Q();case`rooms`:return tt();case`team`:return nt();case`settings`:return rt()}}function Be(){let e=P?.repos??[];if(!e.length){J(`Sessions`,`Every agent currently working a repository your team has connected, and the event log behind them.`,`The log fills in on its own the moment a daemon on an enrolled repository reaches the relay.`);return}j.innerHTML=`
    ...
    </div>`,e(j),k(`#setup-mint`).addEventListener(`click`,async()=>{let e=k(`#setup-mint`),r=k(`#setup-err`);r.hidden=!0,e.disabled=!0;try{let r=k(`#setup-label`).value.trim()||`laptop`,i=await E(`/api/tokens`,{method:`POST`,body:JSON.stringify({label:r})});k(`#setup-key`).innerHTML=`<div class="reveal">
        <div class="lbl">Your key. This is the only time it is readable; the join command in step 3 now carries it.</div>
        <div class="val">${t(i.token)}</div></div>`,k(`#setup-join`).querySelector(`code`).textContent=`knoot join ${i.token} --relay ${n}`;let a=k(`#step-key`);a.classList.remove(`now`,`later`),a.classList.add(`done`),e.textContent=`Mint another`,e.classList.add(`quiet`),await $(),U()}catch(e){r.textContent=e.message,r.hidden=!1}finally{e.disabled=!1}}),K(),r.repos||(G=setInterval(async()=>{try{await $(),P?.repos?.length&&(I=P.repos[0].repo,W())}catch{}},4e3))}function Y(){let e=k(`#log-rows`),t=k(`#log`);F?.close(),F=new Me(n=>We(n,e,t),()=>{Ge(),e.dataset.seeded||Ue(e,t)},e=>{let t=k(`#conn`);t.textContent=e===!0?`live`:e===!1?`reconnecting`:`idle`,t.className=`state`+(e===!0?` live`:e===!1?` off`:``)}),F.open(I)}function He(e,n){let r=Ne[e.type]??`plain`,i=e.ts?new Date(e.ts).toLocaleTimeString([],{hour12:!1}).slice(0,8):``;return`<div class="row${r===`blocked`?` is-blocked`:``}${n?` enter`:``}">
    <span class="t">${t(i)}</span><span class="u">${t(e.user??``)}</span>
    <span class="k ${r}">${t(e.type)}</span><span class="d">${e.path?`<a href="#history" data-why="${t(e.path)}">${t(e.path)}</a>${t(D(e).replace(e.path,``))}`:t(D(e))}</span></div>`}function Ue(e,t){let n=F?.events??[];e.innerHTML=n.length?n.slice(-300).map(e=>He(e,!1)).join(``):`<div class="empty">Connected. Nothing has happened in this repository yet.</div>`,e.dataset.seeded=`1`,t.scrollTop=t.scrollHeight}function We(e,t,n){let r=n.scrollTop+n.clientHeight>=n.scrollHeight-30;for(t.querySelector(`.empty`)&&(t.innerHTML=``),t.insertAdjacentHTML(`beforeend`,He(e,!0));t.children.length>300;)t.removeChild(t.firstChild);r&&(n.scrollTop=n.scrollHeight)}function Ge(){let e=k(`#presence`);if(!e||!F)return;let n=[...F.sessions.values()],r=new Set(F.events.slice(-60).filter(e=>e.type===`claim_denied`).map(e=>e.session));e.innerHTML=n.length?`<table class="rows">
        ...
        <tbody>${n.map(e=>{let n=F.claims.filter(t=>t.session===e.session).map(e=>e.path),i=r.has(e.session)&&!n.length,a=n.length?`holds`:i?`holds blocked`:`holds none`,o=n.length?n.join(`  `):i?`blocked, waiting`:`nothing`;return`<tr><td class="mono">${t(e.user??e.session.slice(0,8))}</td>
            ...
            <td class="${a}">${t(o)}</td></tr>`}).join(``)}</tbody></table>`:``;let i=n.length,a=F.claims.length,o=r.size,s=k(`#counts`);s&&(s.innerHTML=`<span><b>${i}</b>session${i===1?``:`s`}</span><span><b>${a}</b>claim${a===1?``:`s`}</span>`+(o?`<span class="blocked"><b>${o}</b>blocked</span>`:``))}function Ke(e){return`<select class="picker" id="${e}" aria-label="Repository">${(P?.repos??[]).map(e=>`<option value="${t(e.repo)}"${e.repo===I?` selected`:``}>${t(e.repo)}</option>`).join(``)}</select>`}function qe(e,t){let n=k(e);n&&((!I||!(P?.repos??[]).some(e=>e.repo===I))&&(I=n.value),n.value=I,n.addEventListener(`change`,()=>{I=n.value,t()}))}var X={facts:`fact`,repo_cache:`derived`,session_context:`session`};function Je(){if(!(P?.repos??[]).length){J(`Memory`,`What the team has written down about each repository, and who wrote it.`,`Memory is scoped to a repository, so the first fact can only follow the first connection.`);return}j.innerHTML=`
    <div class="page">
      <div class="page-head">
        <div>
          <h1>Memory</h1>
    ...
    </div>`;let e=null,n=()=>{let n=k(`#mem-body`),r=k(`#mem-state`);if(!e)return;let i=(k(`#mem-q`).value||``).trim().toLowerCase(),a=e.items.filter(e=>!i||JSON.stringify(e).toLowerCase().includes(i)),o=e.items.length;if(r.textContent=e.readable?`${o} ${o===1?`entry`:`entries`}${e.unreadable?`, ${e.unreadable} unreadable`:``}`:`${o} sealed`,!e.readable){n.innerHTML=`<div class="empty">This relay seals memory end to end (<code>${t(e.provider)}</code>), so the console can see that ${o} ${o===1?`entry exists`:`entries exist`} but not what ${o===1?`it says`:`they say`}. Read it on an enrolled machine with <code>knoot recall</code>.</div>${o?`<table class="rows"><thead><tr><th>Kind</th><th>Written by</th><th>When</th></tr></thead><tbody>${a.map(e=>`<tr><td>${t(X[e.kind]??e.kind)}</td><td>${t(e.author_email)}</td><td class="dim">${t(O(e.created_ts))}</td></tr>`).join(``)}</tbody></table>`:``}`;return}if(!a.length){n.innerHTML=`<div class="empty">${o?`Nothing matches that filter.`:`Nothing written down for this repository yet. The first fact usually pays for itself the next time somebody opens the file it names.`}</div>`;return}n.innerHTML=`<div class="facts">${a.map(Ye).join(``)}</div>`;for(let e of n.querySelectorAll(`[data-why]`))e.addEventListener(`click`,t=>{t.preventDefault(),Xe(e.dataset.why)})},r=async()=>{k(`#mem-body`).innerHTML=`<div class="empty">Loading.</div>`;try{e=await E(`/api/memory?repo=${encodeURIComponent(I)}`),n()}catch(e){k(`#mem-body`).innerHTML=`<div class="empty">Could not load memory: ${t(e.message)}</div>`}};qe(`#mem-repo`,()=>{r()}),k(`#mem-q`).addEventListener(`input`,n),r()}function Ye(e){let n=X[e.kind]??e.kind,r=e.kind===`session_context`?e.derived?`appears to be working on`:`plan`:e.name??``;return`<article class="fact${e.stale?` is-stale`:``}">
    <div class="fact-head">
      <span class="fact-kind ${t(e.kind)}">${t(n)}</span>
      <h3>${t(r)}</h3>
      <span class="fact-meta">${t(e.author_email)} · ${t(O(e.created_ts))}${e.area&&e.area!==`/`?` · ${t(e.area)}`:``}</span>
    </div>
  ...
  </article>`}var Z=``;function Xe(e){Z=e,location.hash=`#history`}function Ze(){if(!(P?.repos??[]).length){J(`History`,`Why a file is the way it is: who took it, who wrote it, who was blocked, and what was said.`,`History is read back from the log, and the log starts with the first connection.`);return}j.innerHTML=`
    <div class="page">
      <div class="page-head">
        <div>
          <h1>History</h1>
    ...
    </div>`;for(let e of j.querySelectorAll(`[data-repo]`))e.addEventListener(`click`,()=>{I=e.dataset.repo});e(j)}function Q(){let r=P?.tokens??[],i=r.filter(e=>!e.revoked).length,a=P?.members??[],o=a.filter(e=>e.unassigned),s=P?.me;j.innerHTML=`
    <div class="page">
      <div class="page-head">
        <div>
          <h1>Agent keys</h1>
    ...
    </div>`,e(j),k(`#mint-go`).addEventListener(`click`,async()=>{let r=k(`#mint-go`),i=k(`#tok-err`);i.hidden=!0,r.disabled=!0;try{let r=k(`#mint-label`).value.trim(),i=k(`#mint-member`)?.value,a=await E(`/api/tokens`,{method:`POST`,body:JSON.stringify(i?{label:r,member:i}:{label:r})});k(`#mint-out`).innerHTML=`<div class="reveal">
        <div class="lbl">New key. This is the only time it is readable.</div>
        <div class="val">${t(a.token)}</div></div>
        <div class="cmd-row"><code>knoot join ${t(a.token)} --relay ${t(n)}</code><button class="copy" type="button">Copy</button></div>`,e(k(`#mint-out`)),await $()}catch(e){i.textContent=e.message,i.hidden=!1}finally{r.disabled=!1}});for(let e of j.querySelectorAll(`[data-attach]`))e.addEventListener(`click`,async()=>{e.disabled=!0;try{await E(`/api/members/attach`,{method:`POST`,body:JSON.stringify({from:e.dataset.attach})}),await $(),Q()}catch(t){let n=k(`#tok-err`);n.textContent=t.message,n.hidden=!1,e.disabled=!1}});for(let e of j.querySelectorAll(`[data-revoke]`))e.addEventListener(`click`,async()=>{if(confirm(`Revoke this key? The machine using it stops coordinating. It fails open, so its agents keep working alone.`)){e.disabled=!0;try{await E(`/api/tokens/${encodeURIComponent(e.dataset.revoke)}/revoke`,{method:`POST`}),await $(),Q()}catch(t){let n=k(`#tok-err`);n.textContent=t.message,n.hidden=!1,e.disabled=!1}}})}function tt(){let e=P?.rooms??[],n=P?.members??[],r=P?.repos??[],i=P?.me,a=i?.role===`owner`||i?.role===`admin`,o=e=>e.repo===`*`&&e.area===`/`?`every repository`:`${e.repo}:${e.area}`;j.innerHTML=`
    <div class="page">
      <div class="page-head">
        <div>
          <h1>Rooms</h1>
      ...
      ${n?`<div class="panel">
        ...
        <div class="panel-body">
          ...
          <div class="err" id="add-err" hidden></div>
    ...
    </div>`,e(j);let i=e=>r.find(t=>t.email.toLowerCase()===e.toLowerCase()),a=async()=>{try{let e=await ke();k(`#invites`).innerHTML=e.length?`<table class="rows"><thead><tr><th>Email</th><th>Role</th><th>Expires</th><th></th></tr></thead>
           <tbody>${e.map(e=>`<tr>
           ...
           </tr>`).join(``)}</tbody></table>`:`<div class="empty">None outstanding.</div>`;for(let e of j.querySelectorAll(`[data-inv-revoke]`))e.addEventListener(`click`,async()=>{e.disabled=!0;try{await Ae(e.dataset.invRevoke),await a()}catch(t){alert(t.message),e.disabled=!1}})}catch(e){k(`#invites`).innerHTML=`<div class="empty">Could not load invitations: ${t(e.message)}</div>`}},o=async()=>{try{let e=r.filter(e=>!e.unassigned).map(e=>({user_id:``,email:e.email,role:e.role,created_at:``}));k(`#members`).innerHTML=e.length?`<table class="rows"><thead><tr><th>Email</th><th>Role</th><th>Joined</th><th>Keys</th><th></th></tr></thead>
    ...
    </div>`,e(j),k(`#pw-go`).addEventListener(`click`,async()=>{let e=k(`#pw-err`),t=k(`#pw-ok`);e.hidden=!0,t.hidden=!0;let n=k(`#new-password`).value;if(n.length<8){e.textContent=`Use at least 8 characters.`,e.hidden=!1;return}let{error:r}=await C.auth.updateUser({password:n});if(r){e.textContent=r.message,e.hidden=!1;return}t.textContent=`Password changed.`,t.hidden=!1,k(`#new-password`).value=``})}async function $(){P=await E(`/api/team`)}async function it(){V()}addEventListener(`hashchange`,()=>{if(A.hidden){R=location.hash===`#signup`?`signup`:`signin`,z();return}W()}),setInterval(async()=>{if(!A.hidden&&M)try{let e=(P?.repos??[]).map(e=>e.repo).join();await $();let t=(P?.repos??[]).map(e=>e.repo).join();(e!==t&&H()!==`sessions`||e!==t&&!I)&&W(),U()}catch{}},2e4),it();